...
async fn test_native_global_contract_deployment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Session vault code size: {} bytes", session_vault_code.len());
                        ...
                        if init_outcome.is_success() {
                            ...
                            match metadata_result {
                                Ok(metadata_view) => {
                                    ...
                                    let metadata: serde_json::Value = serde_json::from_slice(&metadata_view.result)?;
                            ...
                            println!("⚠️  Contract initialization failed: {:?}", init_outcome.failures());
                        }
                    }
                    Err(e) => {
...
async fn test_factory_validates_contract_hash() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        Ok(outcome) => {
            ...
            assert!(
                error_msg.contains("Invalid contract code") || 
                error_msg.contains("Expected hash"),
